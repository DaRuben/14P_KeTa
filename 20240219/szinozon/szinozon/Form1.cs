...
namespace szinozon
{
    public partial class Form1 : Form
    {
        ...
        {
            InitializeComponent();
            if (File.Exists("../../../szinek.txt"))
            {
                List<string> sorok = File.ReadAllLines("../../../szinek.txt").ToList();
                txtbol = new List<Color>();
                szinek = new List<Color>();
                szinek2 = new List<Color>();

                foreach (string sor in sorok)
                {
                    string[] sz1 = sor.Split(' ');
                    foreach (string szin in sz1)
                    {
                        Color szinObj = Color.FromName(szin);
                        txtbol.Add(szinObj);
                        if (txtbol.Count > 6)
                        {
                            txtbol.RemoveAt(6);
                ...
                szinek.AddRange(txtbol);
                szinek2.AddRange(txtbol);
            }
            else
            {
                szinek = new List<Color>
